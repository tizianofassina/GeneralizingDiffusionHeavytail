...
class GMMFlow(eqx.Module):
    ...
    def __init__(self, dim: int, sigma: float, tail_index: int, flow_layers: int,
        ...
        # Base distribution: Gaussian (no heavy tails for GMM)
        base_dist = flow_dist.Transformed(
            base_dist=flow_dist.Normal(
                loc=jnp.zeros((dim,)),
                scale=jnp.ones((dim,)) * (sigma if sigma != 0. else 1.0),
            ),
...
if __name__ == "__main__":
    ...
    DATA_INPUT_DIR = "data_gmm/dim_2"

    print("\n" + "=" * 80)
    print("GMM Flow Training")
    print("=" * 80)
    ...
    os.makedirs(f"data_gen_noised_gmm/dim_{dim}", exist_ok=True)
    ...
    for rng_idx, (train_size, inf, sig) in enumerate(configs):
        ...
        base_name = (f"flow_gmm_dim{dim}_sig{sig}_tail{tail_index}_layers{flow_layers}"
                     f"_w{nn_width}_d{nn_depth}_inf{inf}_n{train_size}_ep{n_epochs}_{TRAIN_MODALITY}")
        np.save(f"data_gen_noised_gmm/dim_{dim}/samples_flow_{base_name}.npy", np.asarray(flow_samples))
